...
void main() {
  ...
  group('developer identity', () {
    test('links point at the real profiles', () {
      expect(AppInfo.githubUrl, 'https://github.com/CodeShowOff');
      expect(AppInfo.linkedinUrl, '');
      expect(AppInfo.sponsorUrl, '');
      // The public repo and website — a wrong link here ships to every phone.
      expect(AppInfo.repoUrl, 'https://github.com/CodeShowOff/XPENC');
      expect(AppInfo.websiteUrl, '');
    });

    test('every advertised link is an absolute https URL (if provided)', () {
      for (final url in [
        AppInfo.githubUrl,
        AppInfo.linkedinUrl,
        AppInfo.sponsorUrl,
    ...
    test('advertised emails are plain addresses, not the internal account (if provided)',
        () {
