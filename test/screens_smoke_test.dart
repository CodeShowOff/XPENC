...
void main() {
  ...
  testWidgets('About renders the brand and the developer links', (
    ...
    await tester.scrollUntilVisible(find.text('CodeShowOff/XPENC'), 240);
    if (AppInfo.websiteUrl.isNotEmpty) {
      expect(find.text(AppInfo.websiteUrl.replaceAll('https://', '')), findsOneWidget);
    }
    expect(find.text('CodeShowOff/XPENC'), findsOneWidget);
    await unmount(tester);
  });
